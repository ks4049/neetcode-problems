...
class Solution {
    public int maxAreaOfIsland(int[][] grid) {
      ...
      for(int i=0; i<grid.length; i++) {
        for(int j=0; j<grid[i].length; j++) {
          ...
          if(grid[i][j] == 1 && !visitedNodes.contains(currentNode)) {
            int beforeVisitingSize = visitedNodes.size();
          	numIslandsUtil(grid, i, j, visitedNodes);
            if(maxIslandArea < visitedNodes.size()-beforeVisitingSize) {
              maxIslandArea = visitedNodes.size()-beforeVisitingSize;
            }
          }
        }
      }
  	...
  	private void numIslandsUtil(int[][] grid, int row, int col, Set<String> visitedNodes) {
